...
coco_class_index = [0,1]
coco_class_labels = ('bcg','person')
...
class COCODataset(Dataset):
    def __init__(self, 
        ...
        if image_set == 'train2017':
            self.json_file='instances_train.json'
        elif image_set == 'val2017':
            self.json_file='instances_val.json'
        elif image_set == 'test2017':
            self.json_file='image_info_test-dev.json'
        else:
    ...
    def pull_image(self, index):
        ...
        img_file = os.path.join(self.data_dir, self.image_set,
                                str(img_id).zfill(4) + '.png')


        if self.json_file == 'instances_val.json':
            img_id = self.ids[index]
            img_file = os.path.join(self.data_dir, 'val2017',
                                    str(img_id).zfill(4) + '.png')
