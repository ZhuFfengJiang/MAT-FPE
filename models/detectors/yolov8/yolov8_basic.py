...
def get_conv2dDCN(c1, c2, k, p, s, d, g, bias=False):
    if c1==c2:
        conv = DCNv4(channels=c2,kernel_size=k, stride=s, padding=p, dilation=d, groups=g, bias=bias)
    else:
...
class ConvDCN(nn.Module):
    def __init__(self, 
                 ...
                 depthwise=False):
        super(ConvDCN, self).__init__()
        convs = []
        print("using DCNV4=============USING DCNV4==============USING DCNV4==============USING DCNV4==============")
        add_bias = False if norm_type else True
        if depthwise:
            convs.append(get_conv2dDCN(c1, c1, k=k, p=p, s=s, d=d, g=c1, bias=add_bias))
            # depthwise conv
            if norm_type:
            ...
            # pointwise conv
            convs.append(get_conv2dDCN(c1, c2, k=1, p=0, s=1, d=d, g=1, bias=add_bias))
            if norm_type:
            ...
            convs.append(get_conv2dDCN(c1, c2, k=k, p=p, s=s, d=d, g=1, bias=add_bias))
            if norm_type:
                convs.append(get_norm(norm_type, c2))
            if act_type:
                convs.append(get_activation(act_type))
